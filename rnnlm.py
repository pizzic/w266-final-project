...
class RNNLM(object):
    ...
    def BuildCoreGraph(self):
        ...
        with tf.name_scope("Embedding_Layer"):
            self.W_in_ = tf.Variable(tf.random_uniform([self.V, self.H], 0.0, 1.0), name="W_in_")
            self.x_ = tf.reshape(tf.nn.embedding_lookup(
                self.W_in_, self.input_w_), [self.batch_size_, self.max_time_, self.H], name = "x_")
        ...
        with tf.name_scope("Output_Layer"):
            self.W_out_ = tf.Variable(tf.random_uniform([self.H, self.V], 0.0, 1.0), name="W_out_")
    ...
    def BuildSamplerGraph(self):
        ...
        self.pred_samples_ = tf.reshape(
            tf.multinomial(tf.reshape(self.logits_, [-1, self.V]), 1),
